...
LINE3_REVIEW_TABLE = "dbo.readmrz_ocr_line_items3"


def line3_paths() -> dict[str, Path]:
    env = read_env_file()
    dataset_dir = Path(
        env_value(env, "READMRZ_EVISA_LINE3_DATASET_DIR", str(Path.cwd() / "generated_datasets" / "mrz_ocr_lines3"))
    ).expanduser().resolve()
    image_base_dir = Path(env_value(env, "READMRZ_EVISA_LINE3_IMAGE_BASE_DIR", str(dataset_dir / "images"))).expanduser().resolve()
    return {
...
def image_to_base64(path: Path) -> tuple[str, str, int, int]:
    ...
    if image is None:
        raise ValueError(f"Cannot read OCR line3 image: {path}")
...
def previous_label_id(cursor: Any, before_id: int) -> int | None:
    if before_id <= 0:
        cursor.execute("SELECT ISNULL(MAX(label_item_id) + 1, 0) FROM dbo.readmrz_ocr_line_items3")
        row = cursor.fetchone()
...
def get_next_ocr_line3_review_item(after_id: int = 0) -> dict[str, Any]:
    paths = line3_paths()
...
def get_previous_ocr_line3_review_item(before_id: int = 0) -> dict[str, Any]:
    paths = line3_paths()
    with connect() as connection:
        cursor = connection.cursor()
...
def submit_ocr_line3_review_decision(label_item_id: int, decision: str, lines: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    with connect() as connection:
        ...
        if not rows:
            raise KeyError(f"OCR line3 group not found: {label_item_id}")
        ...
        for row in rows:
            ...
            cursor.execute(
                """
                UPDATE dbo.readmrz_ocr_line_items3
                SET review_status = ?,
    ...
    next_response = get_next_ocr_line3_review_item(label_item_id)
    return {
        "status": "ok",
